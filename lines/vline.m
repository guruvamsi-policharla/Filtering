...
if length(x)>1  % vector input
    ...
    if length(label)&& length(x)>1
        for kk=1:length(x)-1
            text((log(x(kk))+log(x(kk+1))/2) ,y(1)+0.9*(y(2)-y(1)),label,'color',get(h,'color'))
        end
     elseif length(label)
        
        xx=get(gca,'xlim');
        xrange=log(xx(2)-xx(1));
        xunit=log((x-xx(1)))/xrange;
        if xunit<0.8
            text(x+0.05*xrange,y(1)+0.9*(y(2)-y(1)),label,'color',get(h,'color'))
        else
            text(x-.05*xrange,y(1)+0.9*(y(2)-y(1)),label,'color',get(h,'color'))
